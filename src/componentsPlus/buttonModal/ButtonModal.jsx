    ...
    <>
      ...
      >
        ...
        <Button className="btnExit" onClick={handleClose}>
          ...
          <Typography id="modal-modal-title" variant="h6" component="h2" sx={{fontWeight: 900}}>
          ...
          <div className="form-exp">
            ...
            serie(n) = (primo({number}+3) * triangular({number}-1))/ fibonacci({number}-2)
            </label>
          </div>
            ...
            <p>serie(n) = ({formula.p} * {formula.t}) / {formula.f}</p>
            <p>serie(n)= ({formula.p * formula.t}) / {formula.f}</p>
            <div className="form-exp">
            <label className="color-orange">
                serie(n)= {result}
